...
\>\>איזה מהבאים הוא עיקרון ביטחוני מקובל?|| בדוק את כל האפשרויות שלהלן שהן עקרונות אבטחה מקובלים, ואל תבדוק אותן אחרת.<<
...
2. הקפד להוריד ולהתקין את התוכנה בצורה אמינה:
    ...
    4. נסה להימנע משימוש בצינור למעטפת (כגון **תלתל ... | sh**) כדי להוריד ולהתקין תוכנה. ברור שאינך יכול להוריד ולעכב את ההתקנה כאשר אתה משתמש בצינור למעטפת. בנוסף, תוקפים החותרים תחת אתר מקור יכולים לזהות בקשה מצינור למעטפת ולחתור באופן סלקטיבי תחת משתמשי pipe-to-shell, אשר מעצם הגדרתם אינם בודקים את מה שהם מורידים (ראו  ספרו [של פיל "זיהוי השימוש ב-curl | bash" צד שרת"](https://www.idontplaydarts.com/2016/04/detecting-curl-pipe-bash-server-side/)). השימוש בצינור למעטפת הופך את החתרנות באתר המקור לקשה הרבה יותר לזיהוי ולנטרול. זה גם מקשה על הבנת הגרסה בפועל שהורדת והתקנת באופן סמכותי - כך שאיבדת למעשה שליטה מסוימת בגרסה, ואינך יכול לסמוך על אחרים כדי שתוכל לקבוע מה קרה. כן, התוכנית המותקנת יכולה לדווח על גירסה, אך תוכניות יכולות לדווח על כל מספר ויכולות לדווח על אותו מספר עבור גירסאות בפועל שונות. בקיצור, הסיכונים שלך גדלים אם אתה משתמש צינור למעטפת.
...
> דוגמה לפגיעות אבטחה הנגרמת על-ידי טיפול גרוע בשגיאות היא [CVE-2014-1266](https://cve.mitre.org/cgi-bin/cvename.cgi?name=CVE-2014-1266), המכונה בדרך כלל "*goto fail; goto להיכשל;* " פגיעות. זו הייתה נקודת תורפה ביישום אפל של פרוטוקול SSL/TLS בגרסאות רבות של מערכות ההפעלה שלה. הבעיה היתה שנייה (כפול) "**goto להיכשל;** " הצהרה בפונקציה **SSLVerifySignedServerKeyExchange**, כדלקמן:
...
בשפות רבות יש הצהרות או ביטויים "טוענים" כדי לומר משהו שאמור ** להיות תמיד נכון בזמן ריצה. אלה יכולים להיות שימושיים לבדיקת שפיות של תוכנית בזמן שהיא פועלת. דוגמאות לכך כוללות את  משפט ההטענה של Java, משפט **ההטענה** של  Python, **המאקרו** assert() של  C/C++/Rust  ושיטת **ה-assert()**** של JavaScript Node.js ** . ברוב השפות, אם הטענה נכשלת בזמן ריצה, אז נזרק חריג.
...
שתי מדידות פשוטות שיכולות לעזור לך לענות על שאלה זו הן *כיסוי הצהרות* וכיסוי *ענף*:
...
[ ] כיסוי הצהרה מודד את אחוז הענפים שבוצעו על ידי קבוצה כלשהי של בדיקות. {{ נבחר: לא. כיסוי סניף מודד סניפים, כיסויי הצהרות מודד הצהרות. }}
