...
למרבה הצער, שמירה על עקרונות תכנון של מערכת מאובטחת אינה מבטיחה תוכנה מאובטחת. במקום זאת, הם רק מדריך בעל ערך לדרך זו. אתה עדיין צריך *לחשוב*.
...
1. **הרשאות פחותות**<br> כל משתמש (אנושי) ותוכנה צריכים לפעול תוך שימוש במספר ההרשאות הנמוך ביותר האפשרי. עיקרון זה מגביל את הנזק כתוצאה מטעות או התקפה. היא גם מפחיתה את מספר האינטראקציות הפוטנציאליות בין תוכניות מורשות, כך שסביר להניח ששימושים לא מכוונים, לא רצויים או לא ראויים בהרשאות יתרחשו פחות.

2. **גישור הרשאות מלא (גם אי-עקיפה)**<br>כל ניסיון גישה חייב להיבדק; מקם את המנגנון כך שלא ניתן יהיה לערער עליו. מילה נרדפת למטרה זו היא אי-עקיפה.

3. **מנגנון חסכוני (גם פשטות)**<br>המערכת, ובמיוחד החלק שהאבטחה תלויה בו, צריכה להיות פשוטה וקטנה ככל האפשר.

4. **תכנון פתוח**<br>אסור שמנגנון ההגנה יהיה תלוי בידע התוקף. במקום זאת, עליך לפעול כאילו המנגנון ידוע לציבור, ובמקום זאת להסתמך על סודיות של פריטים מעטים יחסית הניתנים לשינוי בקלות כמו סיסמאות או מפתחות פרטיים. תוקף לא אמור להיות מסוגל לפרוץ למערכת רק בגלל שהתוקף יודע איך היא עובדת. "אבטחה באמצעות אלמוניות" בדרך כלל לא עובד.

5. **אל-כשל בברירות מחדל**<br>התקנת ברירת המחדל צריכה להיות ההתקנה המאובטחת. אם לא בטוח שצריך לאפשר משהו, אל תאפשר אותו.
...
7. **המנגנון הכי פחות נפוץ (גם למזער שיתוף)**<br> יש למזער את הכמות והשימוש במנגנונים משותפים. הימנע משיתוף קבצים, ספריות, שימוש בקרנל של מערכת ההפעלה או מחשבים עם משהו שאינך בוטח בו, מכיוון שהתוקפים עלולים לנצל אותם.
...
זכרו, עקרונות העיצוב הם פשוט כללי אצבע. כאשר אנו מפרקים את הבעיה כדי לפתור אותה, עלינו לחשוב על עקרונות אלה, מכיוון שהם יעזרו להנחות אותנו ליצירת תוכנה מאובטחת יותר. ישנם מקרים שבהם יהיו לנו סיבות טובות *לא* ליישם אותם. עקרונות אלה אינם מחליפים את החשיבה - הם עוזרים *להנחות* אותנו כאשר אנו חושבים.
...
עקרון "התכנון הפתוח" אומר שאנחנו לא יכולים לסמוך על התוקפים שלא יודעים איך מערכת עובדת. במקום זאת, עלינו לתכנן את המערכות שלנו כך שיישארו מאובטחות גם כאשר התוקף יודע בדיוק איך הן עובדות.

[הסבר]

### הרשאות פחותות

כבר ציינו כי ההרשאות הפחותות הוא עיקרון חשוב של תכנון מאובטח. הרעיון הבסיסי הוא שכל משתמש (אדם או תוכנית) צריך לפעול תוך שימוש בהרשאות המעטות ביותר האפשריות. באופן כללי, לא נאפשר קריאה או כתיבה של מידע אלא אם כן עלינו לעשות זאת עבור משתמש זה.
...
1. **לא ניתן לתוכנית הרשאות מיוחדות (כאשר זה מעשי)**<br>אם ניתן לעשות זאת, נעשה זאת, מכיוון שזה הטוב ביותר מנקודת מבט של אבטחה. לדוגמה, לינוקס תומכת בהפיכת תוכניות **setuid** או **setgid**, כך שפשוט הפעלת התוכנית מעניקה לתוכנית את ההרשאות של בעליה. אם אנו יכולים להימנע לחלוטין משימוש במנגנון זה, נשקול לעשות את זה, כי זה נותן הרשאות מיוחדות לתוכניות. לעתים קרובות יש חלופות בטוחות יותר; לדוגמה, דרישה מאנשים להתחבר באופן ספציפי עם הרשאות (זוהי המטרה של **SUDO**).

2. **נמזער את ההרשאות המיוחדות שתוכנית מקבלת, כולל מזעור כל הנתונים הנגישים לה**<br>בלינוקס, ייתכן שיש לנו תוכנית עבור (או בשם) קבוצה מיוחדת או משתמש שיש להם רק זכויות ספציפיות, במקום משהו חסוי יותר (כמו root). אם אנו קוראים לממשק שאילתה של מערכת מסד נתונים, נגביל את הזכויות של משתמש מסד הנתונים שבו משתמש היישום. אם מערכת מסד הנתונים שלנו משתמשת ב- SQL, ייתכן שנוכל להשתמש בפקודה SQL GRANT כדי להגביל את ההרשאות שהתוכנית מקבלת. משתמשי Redis עשויים להשתמש בפקודת ACL של Redis כדי להגביל הרשאות.

3. **נוותר לצמיתות על הרשאות בהקדם האפשרי**<br>  לדוגמה, אם אנו משתמשים במזהי קבוצה שמורים של Linux, מזהי משתמשים או ביכולות שלהם, נשחרר לצמיתות הרשאות נוספות אלה בהקדם האפשרי. בדרך זו, אם ההתקפה מתרחשת לאחר מכן, התוקפים אינם יכולים לנצל הרשאות אלה.
...
5. **נחלק את התוכנית למודולים שונים, ונתן הרשאות מיוחדות רק למודול אחד או לכמה מודולים (חלקים מהתוכנית)**  <br>המודול המורשה באופן אידיאלי אפילו לא ייתן אמון מלא בחלקים האחרים של התוכנית שלנו (גם *תכנון חשוד*). אם נעשה זאת, כאשר חלק כלשהו מהתוכנית שלנו יהיה חתרני, זה יגביל את מה שתוקפים יכולים לעשות באופן מיידי. לדוגמה, ייתכן שנפצל את החלק של תוכנית המיישם ממשק משתמש גרפי מחלק אחר בעל הרשאות. מנגנוני הפרדה כמו קונטיינרים, מכונות וירטואליות, seccomp בלינוקס וסוגים שונים של מעטפות אבטחה יכולים לעזור לנו להפריד חלקים של התוכנית, כך שחתרנות של חלק אחד לא בהכרח תשבור אחר. ***היזהרו:*** *נקפיד להגדיר מנגנונים אלה כדי להפריד באופן מאובטח את המודולים, ולהגביל את ההרשאות בכל חלק.* מנגנוני הפרדה אלה לרוב אינם חסינים בפני תקלות, לכן אל תניחו שהשימוש בהם הופך את התוכנית שלך למאובטחת באופן אוטומטי. עם זאת, הם יכולים להפוך את התוכנית שלנו לקשה יותר לתקיפה ועשויים להפחית את הנזק אם התקפה מצליחה.

6. **נמזער (נגביל) את שטח התקיפה** <br>*שטח התקיפה* הוא מערך הפעולות (לדוגמה, ה- API שלו ויציאות הרשת הפתוחות שלו) שתוקפים פוטנציאלים יכולים לגשת אליהן. לדוגמה, אם אנו מאפשרים גישה ציבורית למתודה כלשהי, אז אנו נותנים לכל התוקפים גישה למתודה זו - האם אנו בטוחים שאנו צריכים גישה זו? במידת האפשר, נגביל את הפעולות שתוקפים פוטנציאלים יכולים לגשת אליהן. אם הציבור לא צריך גישה, אל תיתן לציבור גישה. בפרט, נמנע מהשארת פעולות ניפוי שגיאות במערכות פרודקשן שהתוקפים יכולים לגשת אליהן; פעולות ניפוי שגיאות הן מקור נפוץ לבעיות.

7. **נאמת (נבדוק) קלט לפני קבלתו**<br> לא נסתפק בקבלת נתונים מתוקפים פוטנציאלים; נבדוק אותו ביסודיות לפני שנקבל אותו. נדון באימות קלט ביתר פירוט בהמשך. כמובן, עלינו לוודא שהתוקפים אינם יכולים לעקוף אימות קלט זה; זו סוגיה כל כך גדולה שיש לה עיקרון משלה, *גישור מלא*, כלומר *אי-עקיפה*. אנחנו נדבר עליה בהמשך.

8. **ארגז חול של התוכנית שלנו**<br>נפעיל בכוונה את התוכנית (או חלק ממנה) בסביבה עם יכולות מוגבלות במכוון.
...
בעת פיתוח יישומים מבוססי אינטרנט, לא נאפשר למשתמשים לגשת (לקרוא) לקבצים כגון  קבצי **ה include והקונפיגורציה** של השרת. נתונים אלה עשויים לספק בטעות מספיק מידע (למשל, סיסמאות) כדי לפרוץ למערכת. אם אנו משתמש בשרת אינטרנט מסורתי, נשמור את כל מה שאיננו צריכים כדי לשרת ישירות משתמשים מחוץ ל"שורש התיעוד" (**DOCROOT);** בדרך זו, התוקפים אינם יכולים אפילו לבקש את המידע בקלות. נמנע הנגשה של קבצים שידוע לנו שאין להנגיש ישירות (כגון קבצי **include**).

לא נאפשר למשתמשים לכתוב קבצי תצורת מערכת (קונפיגורציה) כברירת מחדל (לדוגמה, קבצי מערכת ב- **etc/** בלינוקס וביוניקס), ובמידת האפשר, נשקול למנוע קריאות גם על ידי משתמשים רגילים. הבעיה היא שמנהלי מערכת מכניסים לעתים קרובות סיסמאות ומפתחות לקבצי תצורה. אם יש סיבות להעניק הרשאות קריאה רחבות יותר לחלק מהמידע על תצורת המערכת (לדוגמה,  ב- **etc/**), נשקול ליצור ספריית תצורת מערכת במקום קובץ תצורת מערכת שבו שם הספריה מסתיים באופן קונבנציונלי ב- **.d**. ספריות תצורת מערכת הן לעתים קרובות טובות יותר בכל מקרה, מכיוון שהן הופכות את השימוש לטריוויאלי עבור מנהלי חבילות להוסיף ולהסיר קבצי תצורה ספציפיים. עבור אבטחה, ספריות תצורת מערכת לא רק מפחיתות את הסיכון לשגיאות, אלא שלקבצים ספציפיים (כגון קבצים עם מפתחות סודיים וסיסמאות) יכולות להיות הרשאות מוגבלות יותר. אם אנו משתמשים בספריית תצורת מערכת, זה פחות בעיה לאפשר למשתמש לקרוא, כי זה הרבה יותר קל להגן על המפתחות הסודיים וסיסמאות.

אם אנו מיישמים API חיצוני (לדוגמה, עם REST או GraphQL), לא נספק פעולת "כתיבה" אלא אם כן אנו מצפים שייעשה בה שימוש. אם אנו מאפשרים כתיבה, ננסה להגביל באופן מקסימלי *את מי* שכותב. לדוגמה, יש בעלים של נתונים ספציפיים ורק לבעלים אפשר לשנות נתונים אלה, במקום לאפשר לכל אחד לשנות דבר מה. אם זה מעשי, נתכנן את התוכנה כך שהיא לא תוכל לכתוב נתונים *גם* אם  הם חתרנים על ידי תוקף (אם כי לעתים קרובות זה לא מעשי).

למרבה הצער, מקובל לנהל הרשאות בצורה לא נכונה. לדוגמה, ישנם מקרים רבים שבהם תוכניות לא הצליחו לשחרר הרשאות בכל המקרים (למשל, משום שהעלאת חריגה דילגה על הקוד שהשמיט הרשאות, או משום שהקוד שהיה אמור לשחרר הרשאות אינו פועל בכל המקרים).
...
בכל פעם שתוכנית מקבלת בקשה, לפחות ממקור שהתוכנית אינה יכולה לתת בו אמון מלא (הוא נמצא מחוץ לגבולות האמון), התוכנית חייבת לבדוק את הבקשה. דוגמאות לבדיקות אבטחה הן בדיקה שהבקשה מאושרת ושהקלט תקף לפני שאנו פועלים על סמך נתונים אלה. עיקרון זה נקרא גם *אי-עקיפה*, משום שהנקודה היא שאסור לתוקפים לעקוף בדיקות ביטחוניות.
...
תכנון מערכת לא מאובטח נפוץ דומה הוא קוד באפליקציות סלולריות לסמארטפון שעושה את כל בדיקות האבטחה לפני שליחת הנתונים שלו לשרת. שוב, איננו יכולים להניח כי בדיקות אבטחה כלשהן ביישום הנייד אכן יבוצעו. תוקפים יכולים לשנות את היישום הנייד, או לכתוב יישום אחר, כדי לעקוף את כל הבדיקות שבוצעו ביישום הנייד. אם אנו כותב יישום נייד לסמארטפון, אנו גם בדרך כלל לא יכולים לסמוך על היישומים האחרים - היישומים האחרים עשויים בעצמם להיות זדוניים!
...
אם אנו כותב דפדפן אינטרנט, לדוגמה, נצטרך לסמוך על שירותי מערכת ההפעלה המקומיים, אך אנו בהחלט לא יכול לסמוך על שרתי אינטרנט מרוחקים ושרירותיים - חלק מאותם שרתי אינטרנט מרוחקים עלולים לשלוח לנו נתונים זדוניים!
...
זה הרבה יותר טוב להפעיל תוכנה שאתה צריך לסמוך עליה, על מערכת שאתה יכול לסמוך עליה; אז התוכנה פשוט עובדת כל הזמן.
...
* אם אנו מנסים לאבטח משחקים במחשב נייד / שולחני, ואנו לא סומכים על המחשב הנייד / השולחני, יש מערכות נגד רמאויות. אבל מערכות נגד רמאויות נשברות באופן שגרתי. עדיף לנו לקיים אירועים פיזיים שבהם כל המחשבים הניידים / המחשבים השולחניים נמצאים בבעלותך.
...
1. **מנגנון חסכוני (גם פשטות)**. המערכת, בפרט החלק שהאבטחה תלויה בו, צריכה להיות פשוטה וקטנה ככל האפשר. זה הופך את החלק הזה של המערכת לקל יותר לבדיקה וקשה יותר לטעות בו. כמובן, תוכנה מודרנית מתבקשת לעתים קרובות לספק הרבה פונקציונליות, אז אנו בדרך כלל לא יכולים לעשות הכל פשוט מאוד, אבל אנו יכולים לפחות לעבוד כדי להפוך את החלק שהאבטחה תלויה בו פשוט ככל האפשר.

2. **תכנון פתוח**. אסור שמנגנון ההגנה יהיה תלוי בבורות של התוקפים. במקום זאת, המנגנון צריך להיות ציבורי, בהתאם לסודיות של פריטים מעטים יחסית (וניתנים לשינוי בקלות) כמו סיסמאות או מפתחות פרטיים. תכנון פתוח מאפשר ביקורת ציבורית נרחבת. תכנון פתוח גם מאפשר למשתמשים לשכנע את עצמם שהמערכת שעומדת לשימוש מספקת. למען האמת, זה לא מציאותי לנסות לשמור על סודיות עבור מערכת שמופצת באופן נרחב; decompilers וחומרה חתרנית יכולים לחשוף במהירות "סודות" במימוש. אחד היתרונות הגדולים של תוכנת קוד פתוח (OSS) הוא שהיא מיישמת טוב יותר את עקרון העיצוב הפתוח; לקוד המקור של OSS יש עיצוב פתוח, המאפשר לכל אחד אחר לסקור אותו ולבצע שינויים כדי לשפר אותו. כמובן, OSS צריך *למעשה* להיסקר עבור זה כדי לעזור, אבל זה  יתרון *פוטנציאלי* חשוב  .

3. **ברירות מחדל בטוחות לכשל (הידועות גם כברירות מחדל אל-כשל)**. התקנת ברירת המחדל צריכה להיות ההתקנה המאובטחת. אם לא בטוח שצריך לאפשר משהו, לא נאפשר אותו. לדוגמה, לא נפיץ תוכנה עם סיסמה ריקה או סיסמת ברירת מחדל; במקום זאת, *נדרוש* להגדיר סיסמה חדשה בעת התקנת התוכנה. בדרך זו, אם מישהו רק מתקין אותה במהירות, לא תהיה לו פגיעות עקב סיסמה ידועה. נודא שהרשאות ברירת המחדל מאובטחות; קטגוריית חולשה [CWE-276](https://cwe.mitre.org/data/definitions/276.html) היא "הרשאות ברירת מחדל שגויות".
...
[!] הפיכת קוד המקור לקשה להבנה (למשל, שימוש מעורפלים) כך שיהיה קשה יותר לזהות פגיעויות. {{ נבחר: לא, אם קוד המקור קשה יותר להבנה, אז יהיה קשה יותר למפתחים להפוך אותו למאובטח מלכתחילה. אתה צריך לשאוף לשמור על המערכת פשוטה ככל האפשר. }}
...
עקרונות תכנון רבים אחרים הוצעו, בהתבסס על בעיות שקרו למערכות קודמות. כעת, נסתכל על כמה עקרונות עיצוב אחרים שאנו צריכים לשקול.

#### היזהרו ממרוץ תהליכים (Race condition)

 *מרוץ תהליכים* מתרחש כאשר ההתנהגות הנכונה של מערכת תלויה ברצף האירועים, אך אין שליטה על רצף זה. מרוצי תהליכים כוללים בדרך כלל תהליך אחד או יותר או ת'רדים שניגשים למשאב משותף, אך גישה מרובה זו לא נשלטה כראוי.
...
לרוב השפות יש שגרה או פקודה ליצור קבצים זמניים באופן מאובטח; השתמשו בהם היכן שהם זמינים. ב- Python מודול tempfile יכול ליצור קבצים זמניים באופן מאובטח. סקריפטים של Shell יכולים להשתמש בפקודה mktemp כדי ליצור קבצים זמניים באופן מאובטח.
...
פגמים קורים! אבל הם לא צריכים להפוך לפגיעויות. במקום זאת, ננסה לתכנן את המערכת שלנו כך שפגם בודד יהיה הרבה פחות סביר שיגרום לסכנה מוחלטת. זהו בעצם יישום ספציפי של עקרון ההרשאות הפחותות, אך אם אנו חושבים באופן ספציפי על הפיכת המערכת לקשה לערעור *גם* כאשר יש בה פגם, אנו עשויים לזהות צעדים אחרים שאנו יכול לנקוט.
