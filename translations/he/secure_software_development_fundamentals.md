...
אין מנגנון קסם לייצור תוכנה מאובטחת. במקום זאת, יש לשקול באופן רציף אבטחה לאורך כל תהליך פיתוח ופריסת התוכנה. התחשבות באבטחה בכל עת, דרך כל תהליכי הפיתוח והפריסה, נקראת לעתים "הגנה לרוחב". בואו נדבר על התהליכים המשמשים לפיתוח תוכנה ופריסה.
...
* *בחירת רכיבים הניתנים לשימוש חוזר* (באילו חבילות/ספריות הניתנות לשימוש חוזר נישתמש). יש להעריך את הרכיבים שיהיו בשימוש, מכיוון שכל אחת מהפגיעויות שלהם עלולה להפוך לפגיעויות של התוכנה שלך. רכיבים אלה שנעשה בהם שימוש חוזר מגיעים ממקום כלשהו, ותלויים באופן טרנזיטיבי ברכיבים אחרים. הסט של כל התלות הזו, כולל מאיפה הם באים ואיך הם מגיעים אליך בסופו של דבר, היא *שרשרת האספקה שלך*.
...
* *אמתו* אותו (כתבו / יישמו בדיקות והשתמשו במנתחי תוכנה (אנלייזרים) כדי לקבל ביטחון שהקוד עושה את מה שהוא אמור לעשות). יש לבדוק כדי לוודא שהמערכת שלך מאובטחת, ולהשתמש בכלים שיעזרו לך למצוא פגיעויות לפני שהתוקפים ימצאו אותן.

* *פרסו* אותו. עיזרו להבטיח שהמשתמשים יוכלו לקבל את הגירסה הנכונה, שהיא מאובטחת כברירת מחדל ושהם יוכלו להפעיל אותה בקלות בצורה מאובטחת.
...
פרקטיקה מומלצת מאוד היא להשתמש באינטגרציה רציפה (CI), שהיא הנוהג למרג'ג' (למזג, merge) 'עותקים עובדים של פיתוח לתוך בראנץ (ענף, branch) ראשי משותף (למשל, אחת לכמה ימים או פעמים רבות ביום). מרג' (merge) שגרתי זה מפחית את הסיכונים של רכיבים לא לעבוד יחד אם האינטגרציה ביניהם התעכבה, וזה דבר טוב. עם זאת, CI מוצלח דורש דרך לקבוע אם הרכיבים אכן עובדים יחד. בעיה זו נפתרת על-ידי שימוש בתהליך CI (פייפליין) - תהליך שפועל בכל פעם שקוד חדש מתמרג'ג' (מתמזג) כדי להבטיח שהוא ניבנה כהלכה ועובר סדרה של בדיקות אוטומטיות ובדיקות אחרות.
...
* אספקה רציפה (Continuous Delivery - CD או CDE) נועדה להבטיח ש *"יישום נמצא תמיד במצב מוכן לסביבת הייצור (פרודקשן), לאחר שעבר בהצלחה בדיקות אוטומטיות ובדיקות איכות [על ידי שימוש בשיטות בדיקה שונות] כדי לספק תוכנה באופן אוטומטי לסביבה דמוית ייצור"* (Mojtaba Shahin, מוחמד עלי באבר ו- Liming Zhu[*, אינטגרציה רציפה, אספקה ופריסה: סקירה שיטתית על גישות, כלים, אתגרים ושיטות עבודה, 2017*](https://arxiv.org/abs/1703.07019)). שימו לב שהתוכנה לא בהכרח משוחררת או נפרסת ללא שלב אישור נפרד.

* פריסה רציפה (Continuous Deployment - CD)  "*הולכת צעד אחד קדימה [מאשר אספקה רציפה] ופורסת את היישום באופן אוטומטי ורציף לסביבות ייצור או סביבת הלקוח"* (Mojtaba Shahin, מוחמד עלי באבר ו-Liming Zhu[*, אינטגרציה רציפה, אספקה ופריסה: סקירה שיטתית על גישות, כלים, אתגרים ופרקטיקות, 2017*](https://arxiv.org/abs/1703.07019)).
...
* גיטאופס (GitOps) "היא דרך ליישם פריסה רציפה עבור יישומי ענן (קלאוד-נייטיב Cloud Native). היא מתמקדת בחוויה ממוקדת-מפתחים בעת תיפעול התשתית, באמצעות כלים המוכרים למפתחים, כולל כלי גיט (Git) ופריסה רציפה.  הרעיון המרכזי של גיטאופס (GitOps) הוא קיום מאגר גיט (Git) המכיל תמיד תיאורים הצהרתיים של התשתית הרצויה בנקודת זמן זו בסביבת הייצור ותהליך אוטומטי כדי להתאים את סביבת הייצור למצב המתואר במאגר. אם רוצים לפרוס יישום חדש או לעדכן יישום קיים, צריך רק לעדכן את המאגר - התהליך האוטומטי מטפל בכל השאר" <https://www.gitops.tech/>. הגדרות דומות כוללות "בתהליך גיטאופס (GitOps) משתמשים במאגרי גיט (Git) כמקור אמת יחיד כדי לספק תשתית כקוד (Infrastructure as Code)". (רד האט,  [*מה זה גיטאופס?*](https://www.redhat.com/en/topics/devops/what-is-gitops) ) ו-"גיטאופס (GitOps) היא מסגרת תפעולית שלוקחת שיטות עבודה מומלצות של DevOps המשמשות לפיתוח יישומים כגון בקרת גרסאות, שיתוף פעולה, תאימות ו-CI/CD, ומחילה אותן על אוטומציה של תשתיות." (GitLab,  [*מה זה GitOps?*](https://about.gitlab.com/topics/gitops/)).

הנושאים המוזכרים מעלה תלויים בבדיקות אוטומטיות ובבדיקות איכות, ומבחינת אבטחה, מה שקריטי הוא שיש לשלב כלים לבדיקת פגיעויות אבטחה ובעיות אבטחה פוטנציאליות בבדיקות האוטומטיות ובבדיקות האיכות הללו. לדוגמה, יש לוודא שכלים שבודקים אם קיימות בעיות אבטחה שונות נמצאים בצינור ה- CI שלך, כך שבעיות אלה יזוהו מוקדם. כלי אבטחה שעלולים לקחת זמן רב עד לקבלת התוצאה עשויים להיות מופעלים במקביל אך לשמש כ"שער" עבור CDE. בהמשך הקורס נדון בהרחבה על כלים לתמיכה באבטחה.
...
**בקיצור:** כלים חשובים, אך הם לא מספיקים. יש לשקול באופן רציף אבטחה לאורך כל הפיתוח והפריסה, על מנת שסיכונים הקשורים לאבטחה יזוהו ויטופלו. יש לשקול כיצד המערכת שלך עלולה להיות מותקפת (זיהוי הסיכונים שלה), לנתח סיכונים כדי לקבוע את הסבירות שהמערכת תנוצל ואת חומרת הסיכון, ולאחר מכן להחליט מה ניתן או צריך לעשות. הוספת כלי אבטחה לתהליך האינטגרציה הרציף שלך בהחלט כלול תחת צעדים אלו, אך כלים אלה יהיו יעילים הרבה יותר אם תהיה מחשבה על אבטחה לאורך כל הפיתוח והפריסה. בהמשך הקורס נעסוק בכיצד יש לעשות זאת. בסופו של דבר נדון בכלים, אבל רק אחרי שנבין מה הכלים עוזרים לנו לעשות.

כמו כן, עלינו להתמקד בשיפור מתמיד, הן של התוכנה עצמה והן של התהליכים שבהם משתמשים כדי לפתח אותה. אם תכנון המערכת מקשה להשתמש ב-API הנוכחיים בצורה מאובטחת, הפכו אותו לקל לשימוש בצורה מאובטחת. חפשו דרכים להקשיח את התוכנה מפני התקפות. שנו את תהליכי האימות על-ידי הוספת כלים חדשים, או שינוי התצורה של כלים קיימים, כדי לזהות יותר ויותר בעיות לפני שהן משוחררות למשתמשים.
...
רשימה זו של חמש פונקציות מפושטת לעתים לשלוש פונקציות בסיסיות: **הגנה**, **זיהוי** ו**תגובה**. בעת שימוש ברשימה פשוטה זו של שלוש פונקציות בסיסיות, זיהוי נחשב כחלק מלהגן, ושיחזור נחשב חלק מתגובה. בקורס זה נשתמש ברשימה המקוצרת של פונקציות בסיסיות.
...
ניתן לקבץ את הרוב המכריע של הפגיעויות לקטגוריות. זו פרקטיקה מאוד שימושית מכיון שברגע שאנחנו מזהים קטגוריות, אנחנו יכולים לקבוע אילו מהן נפוצות ואילו צעדים אנחנו יכולים לנקוט כדי למנוע מפגיעות מסוג זה להתרחש שוב.
...
( ) כל פגיעויות וחשיפות נפוצות (CVE) מוקצים על ידי תאגיד MITRE. {{לא, CVEs מוקצים על-ידי "רשות מספור CVE" (CNA.)}}
...
זכרו שתהליך התכנון, כמו כל תהליך פיתוח תוכנה אחר, אינו חד פעמי. זהו תרחיש נפוץ לנסות ליישם תוכנה, להבין כי התכנון לא עובד, ולאחר מכן לשנות את התכנון. לעתים קרובות יש לשנות תכנון כאשר משנים את מה שהתוכנה עושה. אז תהליך התכנון קורה בכל פעם שנידרש שינוי בפירוק הבעיה שהתוכנה שלך פותרת.

תכנונים מסוימים טובים יותר מאחרים: חלקם קלים יותר לתחזוקה, מהירים יותר וכן הלאה. בפרט, תכנונים מסוימים מאובטחים יותר מאחרים. אין פתרון קסם שמבטיח שהתכנון שלך מאובטח. אבל מניסיונם של אנשים המפתחים תוכנה במשך עשרות שנים, זוהו סט של *עקרונות תכנון* שיכולים לעזור לך לבחור תכנונים טובים על פני רעים.
...
1. **הרשאות פחותות (Least privilege)**<br> כל משתמש (אנושי) ותוכנה צריכים לפעול תוך שימוש במספר ההרשאות הנמוך ביותר האפשרי. עיקרון זה מגביל את הנזק כתוצאה מטעות או התקפה. הוא גם מפחית את מספר האינטראקציות הפוטנציאליות בין תוכנות מורשות, כך שסביר להניח ששימושים לא מכוונים, לא רצויים או לא ראויים בהרשאות יתרחשו פחות.
...
מאוחר יותר, נבחן ביתר פירוט כמה מהעקרונות הללו, משום שיש להם השלכות שאולי אינן ברורות מאליהן. ביחידה הבאה נתחיל בכך שנסתכל על *הרשאות פחותות*.
...
ההרשאה הפחותה ביותר מגבילה את הנזק הפוטנציאלי כתוצאה מתקיפה, וגם מפחיתה את המורכבות של אינטראקציות הקשורות לאבטחה. זה אפילו משתרע על החלקים הפנימיים של תוכנית: רק החלק הקטן ביותר של תוכנית שזקוק להרשאות צריך (באופן אידיאלי) לקבל אותן. כמובן, בשלב מסוים, זה הופך להיות מסובך מדי (ואנחנו גם רוצים לשמור על התוכנית פשוטה ככל האפשר).
...
<br>
 ...
 הרשאות שגויות הן סיבה נפוצה כל כך לפגיעויות אבטחה שהן מדרגות במקום ה 22 ב- 25 ה CWE הגדולות ב 2022 ובמקום ה 15 ב- 25 ה CWE הגדולות ב 2019. זהו [CWE-732](https://cwe.mitre.org/data/definitions/732.html) (*הקצאת הרשאה שגויה עבור משאב קריטי*). הרשאות שגויות גרועות במיוחד אם  הרשאות *ברירת המחדל* אינן מאובטחות; המקרה המיוחד הוא [CWE-276](https://cwe.mitre.org/data/definitions/276.html) (*הרשאות ברירת מחדל שגויות*).
...
למרבה הצער, מקובל לנהל הרשאות בצורה לא נכונה. לדוגמה, ישנם מקרים רבים שבהם תוכניות לא הצליחו לשחרר הרשאות בכל המקרים (למשל, משום שהעלאת או זריקת שגיאה מסוג אקספשן (Exception) דילגה על הקוד שהשמיט הרשאות, או משום שהקוד שהיה אמור לשחרר הרשאות אינו פועל בכל המקרים).

 
 ניהול הרשאות לא תקין הוא גורם כה נפוץ לפגיעויות אבטחה עד כי הוא דורג במקום ה 29 ב- 25 ה CWE הגדולות ב 2021 ובמקום ה 24 ב- 25 ה CWE הגדולות ב 2019. זהו [CWE-269](https://cwe.mitre.org/data/definitions/732.html) (*ניהול הרשאות לא תקין*).
...
אם אנו כותבים דפדפן אינטרנט, לדוגמה, נצטרך לסמוך על שירותי מערכת ההפעלה המקומיים, אך אנו בהחלט לא יכולים לסמוך על שרתי אינטרנט מרוחקים ושרירותיים - חלק מאותם שרתי אינטרנט מרוחקים עלולים לשלוח לנו נתונים זדוניים!
...
מפתחים מסוימים מנסים להריץ קוד במערכות שהם לא יכולים לסמוך עליהן באמצעות טשטוש (obfuscation). כלומר, הם ישתמשו בכלים שמנסים להקשות על הבנת הקוד שנשלח למערכת שהם לא יכולים לסמוך עליה. דוגמה לכך היא שימוש במיניפיקציה של JavaScript ובתקווה שזה יקשה על הבנת קוד הלקוח. אל תעשו את זה! המטרה של מיניפיקציה של JavaScript היא להפחית את מספר הבייטים (bytes) שנשלחים ברשת, לא להסתיר את מה שהקוד עושה או למנוע את שינויו.

כפי שניתן לטשטש (obfuscate) ניתן גם להפוך את הטשטוש (de-obfuscate), וקל להפליא לתוקפים להפוך את טשטוש המידע. קיימים כלים רבים שיכולים לבצע היפוך טשטוש מידע במהירות. עדיף להימנע מניסיון להריץ קוד שאתה צריך לסמוך עליו במערכות שאינך יכול לסמוך עליהן.
...
שגיאה נפוצה במקצת במערכות דמויות יוניקס היא יצירת קבצים זמניים בצורה לא מאובטחת. ניתן ליצור קבצים באופן זמני בספריה שבה תוקפים יכולים להשפיע על היצירה והשמות של קבצים אחרים. אם תוקפים יוצרים תחילה קובץ, ולאחר מכן היישום מבקש "ליצור" קובץ מבלי לבקש שהוא יהיה בלעדי, נעשה שימוש חוזר בקובץ הקיים הנשלט על-ידי התוקף. שימוש פשוט באפשרות המגדירה בלעדיות אינו מספיק, שכן זה עדיין יאפשר מניעת שירות. הפתרון הוא להשתמש בלולאה פשוטה שיוצרת שם קובץ "אקראי" בספרייה המיועדת ולאחר מכן לנסות ליצור אך ורק עם הרשאות מוגבלות מקסימלית.

לרוב השפות יש פונקציה או פקודה ליצור קבצים זמניים באופן מאובטח; השתמשו בהן היכן שהן זמינות. ב- Python מודול tempfile יכול ליצור קבצים זמניים באופן מאובטח. סקריפטים של Shell יכולים להשתמש בפקודה mktemp כדי ליצור קבצים זמניים באופן מאובטח.



 מירוץ תהליכים היא סיבה כה נפוצה לפגיעויות אבטחה שהיא מדורגת במקום ה- 33 ב 25 ה CWE הגדולות של 2019 ובמקום ה 29 ב 25 ה CWE הגדולות של 2021.  *ביצוע מקבילי באמצעות משאב משותף עם סינכרון לא תקין ('מירוץ תהליכים')* הוא [CWE-362](https://cwe.mitre.org/data/definitions/362.html). *קובץ זמני לא מאובטח* הוא [CWE-377](https://cwe.mitre.org/data/definitions/377.html).
...
#### סימכו על ערוצים מהימנים בלבד

באופן כללי, סימכו רק על מידע (קלט או תוצאות) מערוצים מהימנים. לדוגמה, השתמשו ב-  **https://** במקום ב- **http://** בעת יצירת קשר עם שרת, מכיוון שזה מאפשר לבדוק אם לשרת יש אישור הצפנה חוקי עבור אתר זה. באופן כללי עלינו להשתמש ב- **https**, מכיוון שזה ימנע מהתוקפים לחטט או לשנות מידע שהוחלף עם משתמשים אחרים.

#### הפרדת נתונים מ'פעולות'

טריק שימושי לפיתוח תוכנה מאובטחת יותר הוא להפריד נתונים מפעולות (גם תוכנות). במילים אחרות, עלינו להפריד את הנתונים הפסיביים מהתוכנות המבוצעות. בדרך זו, אם תוקפים מצליחים להדליף מידע "נוסף" לנתונים, הדבר לא יגרום להפעלת תוכנית זדונית פוטנציאלית. זוהי בעצם דרך נוספת ליישם את ההרשאה הפחותה ביותר - אל תיתנו לנתונים את הזכות לפעול כתוכנה.

דוגמה טובה לכך היא מדיניות אבטחת התוכן (CSP) הנתמכת על ידי דפדפני אינטרנט מודרניים. CSP מאפשר לך לציין שה- HTML שנשלח הוא 'נתונים' בלבד, *ואינו* מורשה לספק סקריפטים מוטבעים (תוכנות) או עיצובים (שיכולים להיות גם תוכנות) - במקום זאת, ניתן להוריד את הסקריפטים והעיצובים רק ממקומות מהימנים שצוינו. באופן זה, אם תוקפים מצליחים לחתור תחת ה- HTML, התוקפים לא יוכלו לגרום להפעלת תוכנות המסופקות על-ידי התוקפים.
...
2. **האם אנו מעריכים את הגרסה המיועדת?**  יש לוודא שאנו מעריכים את הגרסה המיועדת של התוכנה, לא פורק (fork) אישי ולא כזה אשר בשליטת תוקף. טכניקות אלה מסייעות להתמודד עם התקפת "שגיאת הקלדה" או טייפוסקוואטינג (typosquatting) הנפוצה (שבה תוקף יוצר שם "כמעט נכון").
...
3. **האם התלות מתוחזקת?**  תוכנה לא מתוחזקת היא סיכון; רוב התוכנות זקוקות לתחזוקה שוטפת. אם היא לא מאובטחת, סביר להניח שהיא גם לא בטוחה.
    ...
    4. האם יש גרסאות אחרונות או הכרזות מהמתחזק(ים/ות)?
...
7. **האם יש שימוש משמעותי?**  תוכנה עם משתמשים רבים או משתמשים גדולים עשויה להיות בלתי הולמת לשימושך. עם זאת, סביר יותר להניח שתוכנה הנמצאת בשימוש נרחב תציע מידע שימושי על אופן השימוש המאובטח בה, ויותר אנשים ידאגו לאבטחה שלה. בידקו האם שם דומה הוא פופולרי יותר - זה יכול להצביע על התקפת הקלדה, טייפוסקוואטינג (typosquatting).
...
9. **מהי ההערכה שלך לגבי הקוד?**  אפילו סקירה קצרה של קוד המקור של התוכנה, והשינויים שלה לאורך זמן, יכולה לתת לך כמה תובנות. להלן דברים שיש לקחת בחשבון:
